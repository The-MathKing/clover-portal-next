...
import { Lock, ShieldCheck, ChevronRight, Loader2, RefreshCw } from 'lucide-react';
import Link from 'next/link';
import Image from 'next/image';
import ReactMarkdown from 'react-markdown';

export default function AuthorityGeneratorPage() {
  ...
  const resetGenerator = () => {
    setMarkdownOutput('');
    setForm({ businessName: '', industry: '', recentNews: '' });
  };
        ...
        {!isAuthenticated ? (
          <div className="max-w-md mx-auto mt-20">
            <div className="bento-box p-10 text-center">
              ...
              <p className="text-[#a1a1a6] text-sm mb-8">Enter the developer passcode to access the Algorithmic Authority Generator.</p>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="mb-12">
              ...
              <p className="text-[#a1a1a6]">Instantly generate Sentiment Defense articles and High-Authority Citation pitches for an active Tier 2 client.</p>
            </div>
            ...
            <form onSubmit={handleGenerate} className="bento-box p-8 space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-[#a1a1a6]">Recent Business News / Update</label>
                <textarea required value={form.recentNews} onChange={e => setForm({...form, recentNews: e.target.value})} placeholder="e.g. They just opened a new branch in Austin and hired 10 new staff." className="w-full bg-black border border-[#333] rounded-lg px-4 py-3 text-white focus:outline-none focus:border-emerald-500 transition-colors min-h-[100px]" />
              </div>
          ...
          <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-700">
            <div className="flex items-center justify-between border-b border-[#222] pb-6">
              <div>
                <h1 className="text-3xl font-semibold mb-2 text-white">Authority Deliverables: {form.businessName}</h1>
