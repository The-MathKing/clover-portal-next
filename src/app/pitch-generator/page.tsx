...
import { Lock, Presentation, ChevronRight, Loader2, RefreshCw } from 'lucide-react';
...
export default function PitchGeneratorPage() {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPitchMarkdown(data.markdown);
    } catch (err: any) {
        ...
        {!isAuthenticated ? (
          <div className="max-w-md mx-auto mt-20">
            <div className="bento-box p-10 text-center">
              ...
              <p className="text-[#a1a1a6] text-sm mb-8">Enter the developer passcode to access the Sales Pitch Generator.</p>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="mb-12">
              ...
              <p className="text-[#a1a1a6]">Instantly generate a highly-targeted 5-slide GEO pitch deck for your next sales call using Gemini 2.5.</p>
            </div>

            <form onSubmit={handleGenerate} className="bento-box p-8 space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-sm font-medium text-[#a1a1a6]">Target Business Name</label>
              ...
              >
                {loading ? (
                  <><Loader2 className="w-5 h-5 animate-spin" /> Generating Pitch Deck...</>
                ) : (
                  <>Generate Pitch <ChevronRight className="w-5 h-5" /></>
                )}
              </button>
            </form>
          </div>
          ...
          <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-700">
            <div className="flex items-center justify-between border-b border-[#222] pb-6">
              <div>
                <h1 className="text-3xl font-semibold mb-2 text-white">Sales Pitch: {form.businessName}</h1>
                <p className="text-[#a1a1a6]">Optimized for {form.targetAudience}</p>
              </div>
              <button onClick={resetGenerator} className="flex items-center gap-2 px-4 py-2 rounded-full border border-[#333] hover:bg-[#111] transition-colors text-sm font-medium">
                <RefreshCw className="w-4 h-4" /> New Pitch
              </button>
            </div>

            <div className="bento-box p-10 prose prose-invert prose-emerald max-w-none">
              <ReactMarkdown>{pitchMarkdown}</ReactMarkdown>
            </div>
          </div>
        )}
      </main>
